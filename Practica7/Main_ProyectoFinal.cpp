...
float movPelotaZ;
float movPelotaZOffset;
float rotPelotaZ;
float rotPelotaZOffset;
bool avanzaPelotaZ;
...
float rotPalanca = false;
float rotPalancaOffset;
bool avanzaPalanca;

//variables para el resorte
float movResorte;
float movResorteOffset;
float escalarResorte;
float escalarResorteOffset;
bool avanzaResorte;
...
//Variables para la camara libre
glm::vec3 camaraPos = glm::vec3(-8.0f, 20.0f, 0.0f);
glm::vec3 camaraFront = glm::vec3(0.0f, 0.0f, -1.0f);

Window mainWindow;
Window *mainWindowPtr = &mainWindow;
...
float posXanya = 0.0, posYanya = 2.0, posZanya = 0.0;
...
int main()
{
	...
	cameraLibre = Camera(glm::vec3(-8.0f, 20.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), -60.0f, 0.0f, 0.5f, 0.5f);
	...
	skyboxFacesDay.push_back("Textures/Skybox/px.png");
	skyboxFacesDay.push_back("Textures/Skybox/nx.png");
	skyboxFacesDay.push_back("Textures/Skybox/ny.png");
	skyboxFacesDay.push_back("Textures/Skybox/py.png");
	skyboxFacesDay.push_back("Textures/Skybox/pz.png");
	skyboxFacesDay.push_back("Textures/Skybox/nz.png");

	skyboxFacesNight.push_back("Textures/Skybox/pxN2.png");
	skyboxFacesNight.push_back("Textures/Skybox/nxN2.png");
	skyboxFacesNight.push_back("Textures/Skybox/nyN2.png");
	skyboxFacesNight.push_back("Textures/Skybox/pyN2.png");
	skyboxFacesNight.push_back("Textures/Skybox/pzN2.png");
	skyboxFacesNight.push_back("Textures/Skybox/nzN2.png");
	...
	spotLights2[0] = spotLights[2]; //luz fija
	spotLights2[1] = spotLights[0]; //luz para el tablero de pinball
	spotLights2[2] = spotLights[1]; //luz para los flippers 
	...
	movPelotaZ = 0.0f;
	movPelotaZOffset = 0.04f;
	rotPelotaZ = 0.0f;
	rotPelotaZOffset = 5.0f;
	avanzaPelotaZ = true;


	movPalanca = 0.0f;
	movPalancaOffset = 0.01f;
	rotPalancaOffset = 5.0f;
	//avanzaPalanca = false;

	movResorte = 0.0f;
	movResorteOffset = 0.003f;
	//escalarResorte = 1.0f;
	escalarResorteOffset = 1.5f;
	avanzaResorte = true;
	...
	//irrklang::ISoundEngine* engine = irrklang::createIrrKlangDevice(); //sonido espacial
	ISoundEngine* ambiental = createIrrKlangDevice(); //sonido ambiental
	...
	ambiental->play2D("media/PlanBSxP.ogg", true);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// Animación básica de la pelota

		if (avanzaPelotaZ)
		{
			if (movPelotaZ > -16.5f)
			{
				movPelotaZ -= movPelotaZOffset * deltaTime;
				rotPelotaZ += rotPelotaZOffset * deltaTime;
				if (movPelotaZ < -14.0f)
				{
					if (avanzaPelota)
					{
		...
		//Animación de la palanca, se habilitará cuando el mouse de click en la palanca y se deshabilitará cuando se suelte el click
		if (avanzaPalanca)
		{
			if (movPalanca > -0.3f)
			{
				movPalanca -= movPalancaOffset * deltaTime;
				rotPalanca += rotPalancaOffset * deltaTime;
			
				if (avanzaResorte) {
					if (movResorte > -0.2f)
					{
						movResorte -= movResorteOffset * deltaTime;
						//escalarResorte += escalarResorteOffset * deltaTime;
					}
					else
					{
						avanzaResorte = false;
			...
			if (playPelota == true) {
				//resetear variables de la pelota
				movPalanca = 0.0f;
				movResorte = 0.0f;
		...
		if (numCamara == 0) {
			...
			//Recupera la posición de la cámara
			camera->setCameraPosition(camaraPos);
			camera->setCameraDirection(camaraFront);
			...
			camaraPos = camera->getCameraPosition();
			camaraFront = camera->getCameraDirection();
		}
		else if (numCamara == 1) {
			camera = &cameraIsometrica;
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 0.0, movResorte));
		//model = glm::scale(model, glm::vec3(1.0f, 1.0f, escalarResorte));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		resorte.RenderModel();
		...
		//obstaculos en forma de cilindros
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		//animacion básica
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, movPelotaZ));
		model = glm::translate(model, glm::vec3(movPelota, 0.0f, 0.0f));
		model = glm::translate(model, glm::vec3(5.652f, 17.172, 4.064f));
		model = glm::rotate(model, rotacionPelota * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::translate(model, glm::vec3(-5.652f, -17.172, -4.064f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
