...
int main()
{
	...
	pelota = Model();
	pelota.LoadModel("Models/TableroPinball/pelota.obj");
	moneda = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (avanza)
		{
			if (movCoche > -300.0f)
			{
				movCoche -= movOffset * deltaTime; // 0.35*deltaTime este resultado se le decrementa al movimiento del coche, 0 - .35*deltatime = -.35
		...
		model = glm::translate(model, glm::vec3(3.48f, 11.523f, 7.249f));
		model = glm::rotate(model, rotPelota * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::translate(model, glm::vec3(-3.48f, -11.523f, -7.249f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		pelota.RenderModel();
		...
		model = glm::translate(model, glm::vec3(7.35f, -1.55f, -5.0f));
		...
		model = modelaux;
		//trasladar la llanta a su pivote original
